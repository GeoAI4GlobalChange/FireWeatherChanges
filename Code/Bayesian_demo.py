...
from pymc3 import Normal as pm_initialize
...
def initial_parameters():
    ...
    mu_b_fm = pm_initialize("mu_b_fm", mu=0.0, sigma=100)#pm_pool_initialize("mu_b_fm", 1.0)#

    mu_a_fa = pm_pool_initialize("mu_a_fa", 1.0)
    mu_b_fa = pm_initialize("mu_b_fa", mu=0.0, sigma=100)#pm_pool_initialize("mu_b_fa", 1.0)

    mu_a_popu = pm_pool_initialize("mu_a_popu", 1.0)
    mu_b_popu = pm_initialize("mu_b_popu", mu=0.0, sigma=100)#pm_pool_initialize("mu_b_popu", 1.0)

    a_fm = pm_initialize("a_fm", beta=mu_a_fm, dims="eco")  #
    b_fm = pm.Normal("b_fm", mu=mu_b_fm, sigma=100, dims="ecocause")#pm_initialize("b_fm", beta=mu_b_fm, dims="ecocause")#
    a_fa = pm_initialize("a_fa", beta=mu_a_fa, dims="eco")
    b_fa = pm.Normal("b_fa", mu=mu_b_fa, sigma=100, dims="ecocause")# pm_initialize("b_fa", beta=mu_b_fa, dims="ecocause")
    a_popu = pm_initialize("a_popu", beta=mu_a_popu, dims="eco")
    b_popu = pm.Normal("b_popu", mu=mu_b_popu, sigma=100, dims="ecocause")#pm_initialize("b_popu", beta=mu_b_popu, dims="ecocause")
